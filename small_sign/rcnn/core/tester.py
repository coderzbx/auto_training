...
def pred_demo_mask(predictor, test_data, imdb, roidb, result_path, vis=False, thresh=1e-1):
    ...
    for im_info, data_batch in test_data:
        ...
        all_boxes = [None for _ in xrange(imdb.num_classes)]
        all_masks = [None for _ in xrange(imdb.num_classes)]
        label = np.argmax(scores, axis=1)
        label = label[:, np.newaxis]
        ...
        use_global_nms = True
        if use_global_nms:
          print "before:", all_boxes[1:]

          # TODO: apply nms among different classes(exclude the __background__ class)
          expand_all_boxes = []
          for cls in CLASSES:
        ...
        data_dict = dict(zip(data_names, data_batch.data))
        draw_detection_mask(imdb, data_dict['data'], boxes_this_image, masks_this_image, scale, filename)
        i += 1
...
def draw_detection_mask(imdb, im_array, boxes_this_image, masks_this_image, scale, filename):
    import cv2
    import random
    # class_names = ('__background__', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'mcycle', 'bicycle')
    class_names = tuple(imdb.classes)
    class_colors = imdb.class_colors
    color_white = (255, 255, 255)
    im = image.transform_inverse(im_array.asnumpy(), config.PIXEL_MEANS)
    # Scale should be considered here for im, althrough scale is 1.0 when testing
    # change to bgr
    ...
    for j, name in enumerate(class_names):
        ...
        for i in range(len(dets)):
            ...
            mask[mask > 0.5] = 1
            mask[mask <= 0.5] = 0
            mask_color = class_colors[j] # random.randint(0, 255)
            # c = random.randint(0, 2)
            for c in range(3):
              target = im[bbox[1]: bbox[3], bbox[0]: bbox[2], c] + mask_color[c] * mask
