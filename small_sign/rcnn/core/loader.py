...
class TestLoader(mx.io.DataIter):
    ...
    def get_given_index(self):
      cur_from = self.cur
      for i in range(cur_from, self.size):
        roidb = [self.roidb[self.index[i]]]
        #if "4204_20171226022654800949_00_004" in roidb[0]['image']:
        if "4186_20171225070815924433_00_004" in roidb[0]['image']:
          print roidb[0]['image']
          return i
      return 0
...
class AnchorLoaderFPN(mx.io.DataIter):
    ...
    def batches_per_epoch(self):
        return np.ceil(np.single(self.size) / self.batch_size).astype(np.int)
    ...
    def reset(self):
        ...
        if self.shuffle:
            if self.aspect_grouping:
                ...
                np.random.shuffle(self.index)
        all_data, all_label = self.get_batch()
    ...
    def next(self):
        if self.iter_next():
            t_start = time.time()
            all_data, all_label = self.get_batch()
            batch_data = [mx.nd.array(all_data[key]) for key in self.data_name]
            batch_label = [mx.nd.array(all_label[key]) for key in self.label_name]
            provide_data = [(k, v.shape) for k, v in zip(self.data_name, batch_data)]
            provide_label = [(k, v.shape) for k, v in zip(self.label_name, batch_label)]
            logging.info("Get batch time: {}".format(time.time() - t_start))
    ...
    def get_batch(self):
        ...
        cur_from = self.cur
        cur_to = min(cur_from + self.batch_size, self.size)
        return self.get_given_index_batch(cur_from, cur_to)

    def get_given_index_batch(self, cur_from, cur_to):
        roidb = [self.roidb[self.index[i]] for i in range(cur_from, cur_to)]
